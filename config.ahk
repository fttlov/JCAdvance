...
; --- Группа 1: WHEEL ---
; Синий заголовок рамки
MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x155 y" yPos " w480 h270 Center Section", "Wheel")
...
MainGui.Add("Text", "x170 y" (yPos+4) " w250", T("Wheel Gesture DeadZone") ":")
edtDead := MainGui.Add("Edit", "x470 y" yPos " w150", valDead)
CtrlSettings[keyDead] := {type: "edt", ctrl: edtDead, file: ConfigIni, sec: "Motion"}


; --- Группа 2: STICK ---
yPos += 65 ; Переходим к следующей группе с отступом от предыдущей рамки

MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x155 y" yPos " w480 h195 Center Section", "Stick")
MainGui.SetFont("cDefault Norm s10") ; ЯВНЫЙ СБРОС ЦВЕТА И СТИЛЯ НА СТАНДАРТНЫЙ

; Описание (обычный цвет текста)
MainGui.Add("Text", "x170 y" (yPos+20) " w450", T("Map virtual Xbox/KB+M buttons to Right Stick directions. `nOnly for Right Stick Mode: as buttons/as triggers in the 'Analog' tab"))
...
; --- Группа 3: MELEE ---
yPos += 30 ; Переходим к следующей группе с отступом от предыдущей рамки

MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x155 y" yPos " w480 h125 Center Section", "Melee")
MainGui.SetFont("cDefault Norm s10") ; ЯВНЫЙ СБРОС ЦВЕТА И СТИЛЯ НА СТАНДАРТНЫЙ
...
edtForce := MainGui.Add("Edit", "x470 y" yPos " w150", valForce)
...
MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x195 y60 w480 h340 Center Section", T("Gamepad Hotkeys"))
MainGui.SetFont("cDefault Norm s10") ; Сброс на стандартный шрифт

; Подгруппа 1.1: Aiming
MainGui.SetFont("Bold")
MainGui.Add("Text", "xs+15 ys+30 w450", T("Aiming"))
...
; Элегантная тонкая горизонтальная линия-разделитель подгрупп
MainGui.Add("Text", "xs+15 y+20 w450 h2 0x10")

; Подгруппа 1.2: Driving
MainGui.SetFont("Bold")
MainGui.Add("Text", "xs+15 y+18 w450", T("Driving"))
MainGui.SetFont("Norm s10")

; Хоткеи подгруппы Driving
...
; Единственный хоткей этой группы (привязан к началу второй рамки через ys+20)
...
; --- Сноска внизу вкладки ---
MainGui.Add("Text", "x25 y+100 w820 cRed", T("* Gyroscope Recalibration"))
MainGui.Add("Text", "x25 y+5 w820", T("Place the device on a flat surface, press the button, and wait for the beep"))

MainGui.Add("Text", "x25 y+5 w820 cRed", T("** Note:"))
MainGui.Add("Text", "x25 y+5 w820", T("To assign a two-button combination (like R+HOME), you can manually type it into the field above and click Save All"))
; =========================================
...
MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x210 y60 w410 h160 Center Section", T("Behavior and Settings"))
MainGui.SetFont("cDefault Norm s10") ; ЯВНЫЙ СБРОС ЦВЕТА И СТИЛЯ НА СТАНДАРТНЫЙ

AddMappedDropdown(ConfigIni, "Motion", "AimingMode", T("Gyro mode by default"), [T("Right Stick"), T("Mouse")], Map(T("Right Stick"), "0", T("Mouse"), "1"), "xs+15 ys+25")
...
AddInput(XboxIni, "SETTINGS", "MouseSensY", T("Mouse Y"), "150", "xs+110 y+10", 150, 40)
...
AddInput(ConfigIni, "Gamepad", "DeadZoneLeftTrigger", T("DeadZone Left Trigger (0-100)"), , "xs+55 ys+30", 210, 40)
AddInput(ConfigIni, "Gamepad", "DeadZoneLeftStickX", T("DeadZone Left Stick X (0-100)"), , "xs+55 y+8", 210, 40)
AddInput(ConfigIni, "Gamepad", "DeadZoneLeftStickY", T("DeadZone Left Stick Y (0-100)"), , "xs+55 y+8", 210, 40)
AddInput(ConfigIni, "Gamepad", "LinearityLeftStickX", T("Linearity* Left Stick X (0-100)"), "50", "xs+55 y+8", 210, 40)
AddInput(ConfigIni, "Gamepad", "LinearityLeftStickY", T("Linearity* Left Stick Y (0-100)"), "50", "xs+55 y+8", 210, 40)
AddToggle(ConfigIni, "Gamepad", "InvertLeftStickX", T("Invert Left Stick X"), "xs+55 y+10")
AddToggle(ConfigIni, "Gamepad", "InvertLeftStickY", T("Invert Left Stick Y"), "xs+55 y+8")


; --- Группа 2: RIGHT HAND (Правая колонка сверху) ---
; Размещаем сбоку (x425) на той же высоте (y50)
MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x425 y50 w380 h230 Center Section", T("Right Hand"))
MainGui.SetFont("cDefault Norm s10")

AddInput(ConfigIni, "Gamepad", "DeadZoneRightTrigger", T("DeadZone Right Trigger (0-100)"), , "xs+55 ys+30", 210, 40)
AddInput(ConfigIni, "Gamepad", "DeadZoneRightStickX", T("DeadZone Right Stick X (0-100)"), , "xs+55 y+8", 210, 40)
AddInput(ConfigIni, "Gamepad", "DeadZoneRightStickY", T("DeadZone Right Stick Y (0-100)"), , "xs+55 y+8", 210, 40)
AddInput(ConfigIni, "Gamepad", "LinearityRightStickX", T("Linearity* Right Stick X (0-100)"), "50", "xs+55 y+8", 210, 40)
AddInput(ConfigIni, "Gamepad", "LinearityRightStickY", T("Linearity* Right Stick Y (0-100)"), "50", "xs+55 y+8", 210, 40)
AddToggle(ConfigIni, "Gamepad", "InvertRightStickX", T("Invert Right Stick X"), "xs+55 y+10")
AddToggle(ConfigIni, "Gamepad", "InvertRightStickY", T("Invert Right Stick Y"), "xs+55 y+8")


; --- Группа 3: HARDWARE SWAPS (Центрирована, построчно) ---
MainGui.SetFont("cBlue Bold")
MainGui.Add("GroupBox", "x175 y+45 w480 h125 Center Section", T("Hardware Swaps"))
MainGui.SetFont("cDefault Norm s10")

AddToggle(XboxIni, "SETTINGS", "SWAP-STICKS", T("Swap Left and Right Sticks"), "xs+145 ys+30")
AddToggle(XboxIni, "SETTINGS", "SWAP-TRIGGERS", T("Swap Left and Right Triggers"), "xs+145 y+8")

; Настройка режима правого стика (выровнена вровень со свитчами внутри рамки)
valRSMode := IniRead(A_ScriptDir "\" XboxIni, "SETTINGS", "RightStickMode", "0")
MainGui.Add("Text", "xs+115 y+20 w150", T("Right Stick Mode") ":")
...
; --- ВНЕ ГРУПП: Выбор эмулируемого геймпада (Сверху, отступ y60, выровнен по ширине) ---
...
AddInput(ConfigIni, "Motion", "SteeringWheelAngle", T("Steering Wheel Angle"), , "xs+135 ys+40", 160, 50)
AddInput(ConfigIni, "Motion", "LinearityWheel", T("Steering Wheel Linearity"), , "xs+135 y+10", 160, 50)
...
; Имя устройства педалей (теперь через стандартный красивый AddInput!)
...
MainGui.Add("Text", "x25 y+15 w820 cRed", T("* Emulated Controller: DS4 Mode for Nintendo controllers only"))
